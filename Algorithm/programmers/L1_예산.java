...
public class L1_예산 {
    ...
    public static void main(String[] args) {
        L1_예산 program = new L1_예산();
        System.out.println(program.solution(new int[]{1, 3, 2, 5, 4}, 9));
        System.out.println(program.solution(new int[]{2, 2, 3, 3}, 10));
    }

    public int solution(int[] d, int budget) {
        int answer = 0;
        Arrays.sort(d);
        for (int i = 0; i < d.length && 0 < budget; i++) {
            budget -= d[i];
            answer++;
        }
        if(budget < 0) {
